...
// Hàm lấy APi Key từ server qua email
export default async function getApiKey(email) {
  try {
    if (emailRegex(email)) {
      const { data: dataApiKey, res: response } = await client.get(endpoint.apiKey, { email })
      ...
      if (response.ok) {
        ...
        // Lưu apiKey và email vào localStorage khi thành công
